public class FourierFilter {
    ...
    private double fourierCoef = Math.sqrt(2)/dots;
    ...
    public void calculate(double phaseCurrent, double phaseVoltage) {
        ...
        resCurrent = Math.sqrt(Math.pow(sumCosCurrent, 2) + Math.pow(sumSinCurrent, 2))*fourierCoef;
        ...
        resVoltage = Math.sqrt(Math.pow(sumCosVoltage, 2) + Math.pow(sumSinVoltage, 2))*fourierCoef;
        angleVoltage = Math.atan2(sumSinVoltage, sumCosVoltage);
        //System.out.println(phase + " напряжение "+resVoltage+"  угол "+Math.toDegrees(angleVoltage));
